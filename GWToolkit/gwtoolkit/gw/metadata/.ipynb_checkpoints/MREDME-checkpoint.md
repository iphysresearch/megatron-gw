


```python
{ # Copied from Bilby-GWTC-1 analysis settings:
  # https://dcc.ligo.org/LIGO-P2000193/public
    'GW150914': {
        'trigger-time': 1126259462.391, # Trigger time from G184098
        'duration': 8.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1'],
    },
    'GW151012': {
        'trigger-time': 1128678900.4,  # Trigger time
        'duration': 8.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1'],
    },
    'GW151226': {
        'trigger-time': 1135136350.6,
        'duration': 8.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1'],
    },
    'GW170104': {
        'trigger-time': 1167559936.6,
        'duration': 4.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1'],
    },
    'GW170608': {
        'trigger-time': 1180922494.5,
        'duration': 16.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1'],
    },
    'GW170729': {
        'trigger-time': 1185389807.3,
        'duration': 4.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1', 'V1'],
    },
    'GW170809': {
        'trigger-time': 1186302519.7,
        'duration': 4.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1', 'V1'],
    },
    'GW170814': {
        'trigger-time': 1186741861.5,
        'duration': 4.0,
        'post-trigger-duration': 2.0,
        'detectors': ['H1', 'L1', 'V1'],
    },
    'GW170817': {
        'trigger-time': 1187008882.43,
        'duration': 128,
        'post-trigger-duration': 2.0, # not listed in Bilby-GWTC-1
        'detectors': ['H1', 'L1', 'V1'],
    },
    'GW170818': {
        'trigger-time': 1187058327.1,
        'duration': 4.0,
        'post-trigger-duration': 2.0, # not listed in Bilby-GWTC-1
        'detectors': ['H1', 'L1', 'V1'],
    },
    'GW170823': {
        'trigger-time': 1187529256.5,
        'duration': 4.0,
        'post-trigger-duration': 2.0, # not listed in Bilby-GWTC-1
        'detectors': ['H1', 'L1'],
    },
}
```