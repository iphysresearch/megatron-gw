...
Dec 9, 2021
Cloned by He Wang (hewang@mail.bnu.edu.cn)

Updates in cloned version:
 * Add `read_cvmfs_hdf5` and `getstrain_cvmfs` for loading data what have been downloaded by CVMFs.
...
def read_cvmfs_hdf5(filename, ifo, readstrain=True, inj_dq_cache=True):
    """
    ...
    # -- Read the strain
    if readstrain:
        strain = dataFile[f'{ifo}:GWOSC-16KHZ_R1_STRAIN'][...]
    else:
        strain = 0

    gpsStart = int((filename.split('/')[-1].split('-')[-2]))

    gpsLength = int((filename.split('.')[0].split('-')[-1]))
    assert gpsLength == 4096
    gpsEnd = gpsStart + gpsLength

    if inj_dq_cache:
        injmask = dataFile[f'{ifo}:GWOSC-16KHZ_R1_INJMASK'][...]
        dqmask = dataFile[f'{ifo}:GWOSC-16KHZ_R1_DQMASK'][...]

    dataFile.close()
    if inj_dq_cache:
        return strain, gpsStart, gpsEnd, dqmask, injmask
    else:
        return strain, gpsStart, gpsEnd


def getstrain_cvmfs(start, stop, ifo, filelist, inj_dq_cache=True):
    """
    start = 1167559936
    stop = 1167559936+10000
    ifo = 'H1'

    filelist = rl.FileList(directory='/Volumes/O2_H1V1/O2_H1_GWOSC_O2_16KHZ_R1/')
    strain, time, dqmask, injmask = getstrain_cvmfs(start, stop, ifo, filelist)
    """
    first = start - (start % 4096)
    gpsList = np.arange(first, stop, 4096, dtype=int)

    time_array_ = np.empty((len(gpsList), 4096))
    time_array = np.empty((len(gpsList), 4096*16384))
    strain_array = np.empty((len(gpsList), 4096*16384))
    if inj_dq_cache:
        dqmask_array = np.empty((len(gpsList), 4096))
        injmask_array = np.empty((len(gpsList), 4096))
    for i, time in enumerate(gpsList):
        filename = filelist.findfile(time, ifo)
        if inj_dq_cache:
            strain, gpsStart, gpsEnd, dqmask, injmask = read_cvmfs_hdf5(filename, ifo, readstrain=True,
                                                                        inj_dq_cache=inj_dq_cache)
            dqmask_array[i] = dqmask
            injmask_array[i] = injmask
        else:
            strain, gpsStart, gpsEnd = read_cvmfs_hdf5(filename, ifo, readstrain=True,
                                                       inj_dq_cache=inj_dq_cache)
        strain_array[i] = strain
        time_array[i] = np.arange(gpsStart, gpsEnd, 1/16384)
        time_array_[i] = np.arange(gpsStart, gpsEnd)

    cache = (time_array.flatten() >= start) & (time_array.flatten() < stop)
    strain_flatten = strain_array.flatten()[cache]
    time_flatten = time_array.flatten()[cache]

    cache = (time_array_.flatten() >= start) & (time_array_.flatten() < stop)
    if inj_dq_cache:
        dqmask_flatten = dqmask_array.flatten()[cache]
        injmask_flatten = injmask_array.flatten()[cache]
        return strain_flatten, time_flatten, dqmask_flatten, injmask_flatten
    else:
        return strain_flatten, time_flatten
