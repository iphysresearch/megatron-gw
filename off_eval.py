...
def loss_func(loss_mask, sentence_order, clean_signal, output_tensor):
    ...
    averaged_losses = average_losses_across_data_parallel_group(
        [lm_loss])
    # print("loss:{}, average:{}".format(loss,averaged_losses[0]))
...
def forward_step(data_iterator, model):
    ...
    if len(output_tensor)==2:
        ...
        folder = os.path.join('offsource','{}_{}'.format(args.load.strip('/'), args.iteration))
        p = Path(folder)
...
def build_dataset(name, data_prefix, seed):
    from megatron.data.onoff_dataset import OffsourceDataset
    return OffsourceDataset(name=name,
...
def test_datasets_provider():
    ...
    return build_dataset('test', data_prefix= args.data_path, seed=args.seed)
