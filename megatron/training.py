...
def setup_model_and_optimizer(model_provider_func):
    ...
    if len(model) > 1 or mpu.get_pipeline_model_parallel_world_size() > 1:
        ...
        #print_rank_0(mpu.get_pipeline_model_parallel_world_size())
...
def train_step(forward_step_func, data_iterator,
    ...
    #print_rank_0("rank 0: setting grad to zero")
    #print_rank_last("rank last: setting grad to zero")
    # Set grad to zero.
    ...
    #print_rank_0("rank 0: calculate forward output and backward loss")
    ...
    # (BERT and GPT-2).
    #print_rank_0("rank 0: perform embedding backward all-reduce")
    #print_rank_last("rank last: perform embedding backward all-reduce")
    ...
    if (mpu.is_pipeline_first_stage(ignore_virtual=True) or
        ...
        #print_rank_last("unwrapped model: {}".format(unwrapped_model))
        unwrapped_model = unwrap_model(
            unwrapped_model, (torchDDP, LocalDDP, Float16Module))

        if unwrapped_model.share_word_embeddings:
            word_embeddings_weight = unwrapped_model.word_embeddings_weight()
            #print_rank_last("word_embedding_weight:{}".format(word_embeddings_weight))
            if args.DDP_impl == 'local':
                grad = word_embeddings_weight.main_grad
            else:
                grad = word_embeddings_weight.grad
            #print_rank_last("grad shape:{}".format(grad.shape))
            torch.distributed.all_reduce(grad, group=mpu.get_embedding_group())
    timers('backward-embedding-all-reduce').stop()

    # Update parameters.
    #print_rank_0("rank 0: update parameters")
    #print_rank_last("rank last: update parameters")
...
def train(forward_step_func, model, optimizer, lr_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        #print_rank_0("rank 0: start iteration {}".format(iteration))
        #print_rank_last("rank last: start iteration {}".format(iteration))
        ...
        #print_rank_0("rank 0: if evaluation")
        #print_rank_last("rank last: if evaluation")        
        # Evaluation
