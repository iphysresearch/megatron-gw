...
def get_forward_backward_func():
    ...
    if mpu.get_pipeline_model_parallel_world_size() > 1:
        if args.virtual_pipeline_model_parallel_size is not None:
            ...
            #print_rank_0("rank 0 : using forward_backward_pipelining_without_interleaving.")
            #print_rank_last("rank last : using forward_backward_pipelining_without_interleaving.")
...
def forward_backward_pipelining_without_interleaving(forward_step_func, data_iterator,
    ...
    #print_rank_0("rank 0: receive first forward tensor")
    #print_rank_0("rank 0: remaining microbatches: {}".format(num_microbatches_remaining))
    #print_rank_last("rank last: remaining microbatches: {}".format(num_microbatches_remaining))
    if num_microbatches_remaining > 0:
        input_tensor = p2p_communication.recv_forward(timers=timers)
    #print_rank_last("input_tensor: {}".format(input_tensor))
    #print_rank_last("rank last: receive forward from rank 0")
    # Run 1F1B in steady state.
    #print_rank_0("rank 0: start steady forward and backward training")
    #print_rank_last("rank last: start steady forward and backward training")
    for i in range(num_microbatches_remaining):
        ...
        output_tensor = forward_step(forward_step_func, data_iterator, model,
                                     input_tensor, losses_reduced)
        #print_rank_0("rank 0: output_tensor {}".format(output_tensor.shape))
        #print_rank_0("forward only: {}".format(forward_only))
        if forward_only:
            p2p_communication.send_forward(output_tensor, timers=timers)
        else:
            output_tensor_grad = \
                p2p_communication.send_forward_recv_backward(output_tensor,
                                                             timers=timers)
            #print_rank_last("rank last: output_tensor_grad {}".format(output_tensor_grad.shape))

        #print_rank_0("rank 0: finish forward and receive backward")
        ...
        if forward_only:
            ...
            #print_rank_last("rank 0: output_tensor {}".format(output_tensor))
            #print_rank_last("rank 0: input_tensor {}".format(input_tensor))
            input_tensor_grad = \
                backward_step(optimizer, input_tensor, output_tensor,
                              output_tensor_grad)

            #print_rank_last("rank last: input_tensor_grad {}".format(input_tensor_grad))
            if last_iteration:
                input_tensor = None
            ...
            #print_rank_last("rank last: input_tensor {}".format(input_tensor))
            #print_rank_0("rank 0: send backward and receive forward")
    # Run cooldown backward passes.

    #print_rank_last("rank last: finish remaining warmup process")
    #print_rank_0("rank 0: start cooldown backward passes")
    #print_rank_last("rank last: start cooldown backward passes")
    if not forward_only:
        for i in range(num_warmup_microbatches):
            input_tensor = input_tensors.pop(0)
            output_tensor = output_tensors.pop(0)

            #print_rank_0("rank 0: output_tensor {}".format(output_tensor))
            #print_rank_last("rank last: output_tensor {}".format(output_tensor.shape))
            output_tensor_grad = p2p_communication.recv_backward(timers=timers)

            #print_rank_0("rank 0: output_tensor_grad {}".format(output_tensor_grad))
            #print_rank_last("rank last: output_tensor_grad {}".format(output_tensor_grad))
            input_tensor_grad = \
                backward_step(optimizer, input_tensor, output_tensor,
                              output_tensor_grad)

            #print_rank_0("rank 0: input_tensor_grad {}".format(input_tensor_grad))
            #print_rank_last("rank last: input_tensor_grad {}".format(input_tensor_grad))
            p2p_communication.send_backward(input_tensor_grad, timers=timers)

            #print_rank_0("rank 0: finish send backward p2p communication")
            #print_rank_last("rank last: finish send backward p2p communication")
    #print_rank_0("rank 0: finish forward and backward pipeline function")
    #print_rank_last("rank last: finish forward and backward pipeline function")
    return losses_reduced
