...
def forward_step(data_iterator, model):
    ...
    tmp_seed = np.random.randint(10000)
    ...
    print_rank_0('saving evaluation result...')
#    import numpy as np
#    visualize = {'clean': np.array([]), 'noisy': np.array([])}
#    visualize['clean'] = output_tensor[0].cpu().numpy()
#    visualize['noisy'] = noisy_signal.cpu().numpy()
#    from pathlib import Path
#    import h5py
#    p = Path('./vis/')
#    p.mkdir(parents=True, exist_ok=True)
#    data_fn = 'vis-{}.hdf5'.format(np.random.randint(10000))
#    f_data = h5py.File(p / data_fn, 'w')
#    for i in visualize.keys():
#        data_name = i
#        f_data.create_dataset(data_name, data=visualize[i], compression='gzip', compression_opts=9)
#    f_data.close()
#
